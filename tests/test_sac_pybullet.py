...
from envs.bullet_env import RescaleObservationWrapper, RescaleActionWrapper
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("--resume-path", type=str, default=None)#'ckpts/HumanoidDeepMimicWalkBulletEnv-v1/sac/policy.pth')
...
def setup_envs(args):
    ...
    env = make_env(env_name=args.task, norm_obs=False, norm_action=False)
    ...
    train_envs = SubprocVectorEnv(
        [lambda: make_env(env_name=args.task, norm_obs=True, norm_action=False) for _ in range(args.training_num)])
    
    # only render when testing
    render = True if args.render > 0 else False
    render = render and args.watch 
    
    test_envs = SubprocVectorEnv(
        [lambda: make_env(env_name=args.task, renders=render, norm_obs=True, norm_action=False) for _ in range(args.test_num)])
    
    return env, train_envs, test_envs

def test_sac(args: argparse.Namespace = get_args()) -> None:
    env, train_envs, test_envs = setup_envs(args)
